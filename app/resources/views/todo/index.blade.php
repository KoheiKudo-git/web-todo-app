                                ...
                                @if($todo->is_done === false)
                                    <form action="{{ route('todo.done', $todo->id) }}" method="POST">
                                        ...
                                        <button type="submit" class="btn btn-blue">
                                            <i class="fa-regular fa-circle-check pe-2"></i>完了！
                                        </button>
                                    </form>
                                @else
                                    <form action="{{ route('todo.undone', $todo->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" class="btn btn-blue">
                                            <i class="fa-solid fa-circle-xmark pe-2"></i>未完了に戻す！
                                        </button>
                                    </form>
                                @endif
                            </div>
